...
const topLevelItems = [
  ...
  { href: '/dashboard/contact-page', label: 'CONTACT' },
]
...
export default function Sidebar() {
  ...
  return (
    <aside
      className={`fixed top-0 left-0 h-screen bg-[#0a0a0a] border-r border-white/[0.06] flex flex-col transition-all duration-300 z-50 ${
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-5 border-b border-white/[0.06]">
        ...
        <button
          ...
          className={`w-7 h-7 rounded-full bg-white/[0.06] hover:bg-white/10 flex items-center justify-center text-gray-400 hover:text-white transition-colors ${
            collapsed ? 'absolute -right-3.5 top-6 bg-[#0a0a0a] border border-white/[0.06]' : ''
          }`}
        >
        ...
        {!collapsed ? (
          ...
          <Link
            ...
            className={`flex items-center justify-center py-2 rounded-lg transition-colors ${
              pathname === '/dashboard' ? 'text-emerald-400 bg-emerald-500/10' : 'text-gray-500 hover:text-white hover:bg-white/[0.04]'
            }`}
            title="Dashboard"
          >
        ...
        {!collapsed ? (
          <div>
            <button
              ...
              className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm font-semibold transition-all duration-150 ${
                isMainSection ? 'text-white' : 'text-gray-400 hover:text-white'
              }`}
            >
              <span>MAIN</span>
              <svg
          ...
          <Link
            href="/dashboard/hero"
            className={`flex items-center justify-center py-2 rounded-lg transition-colors ${
              isMainSection ? 'text-emerald-400 bg-emerald-500/10' : 'text-gray-500 hover:text-white hover:bg-white/[0.04]'
            }`}
            title="Main"
          >
            ...
            return (
              <li key={item.label}>
                {!collapsed ? (
                  <Link
                    href={item.href}
                    className={`block px-3 py-2 rounded-lg text-sm font-semibold transition-all duration-150 ${
                      isActive
                        ? 'text-emerald-400 bg-emerald-500/10'
                        : 'text-gray-400 hover:text-white hover:bg-white/[0.04]'
                    }`}
                  ...
                  <Link
                    href={item.href}
                    className={`flex items-center justify-center py-2 rounded-lg transition-colors ${
                      isActive ? 'text-emerald-400 bg-emerald-500/10' : 'text-gray-500 hover:text-white hover:bg-white/[0.04]'
                    }`}
                    title={item.label}
                  >
